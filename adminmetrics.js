...
const deadStockSimulation = [
  {
    name: "Story Book - Class 5",
    daysInactive: 42,
    quantity: 38,
    action: "Clearance Sale Recommended"
  },
  {
    name: "Notebook A4 (Old Design)",
    daysInactive: 29,
    quantity: 120,
    action: "Bundle with Fast-Moving Item"
  },
  {
    name: "Color Pencil Set",
    daysInactive: 18,
    quantity: 60,
    action: "Early Promotion Alert"
  }
];

let index = 0;

function renderDeadStockSimulation() {
  const list = document.getElementById("deadStockList");
  const item = deadStockSimulation[index];

  let statusIcon = "🟡";
  if (item.daysInactive >= 30) statusIcon = "🔴";

  list.innerHTML = `
    <li style="margin-bottom:10px">
      ${statusIcon} <b>${item.name}</b><br>
      📦 Qty: ${item.quantity}<br>
      ⏳ Inactive: ${item.daysInactive} days<br>
      💡 Action: <i>${item.action}</i>
    </li>
  `;

  index = (index + 1) % deadStockSimulation.length;
}

// initial render
renderDeadStockSimulation();

// rotate every 4 seconds
setInterval(renderDeadStockSimulation, 4000);
